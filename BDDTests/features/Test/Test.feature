...
Scenario: Check the list of tables exists in a specific Chem domain area
  ...
  """
  Select owner,table_name from ALL_TAB_COLUMNS where owner='CHEM_PROC' order by table_name
  """
  When I execute following query for "Ingest" Schema for table counting:
  """
  SELECT table_schema, table_name as TableName from information_schema.tables where table_schema='chem_proc_ingest' order by table_name
  """
  When I execute following query for "Exposure" Schema for table counting:
  """
  SELECT table_schema, table_name as TableName from information_schema.tables where table_schema='chem' order by table_name
  """
  Then I should get the counts of "Staging","Ingest" against "Exposure" in html format

   # And I should subtract the following list of excluded tables from "Source" results
